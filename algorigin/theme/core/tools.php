...
// CUSTOM BREADCRUMBS
function theme_breadcrumbs_list($wrapper_classes = false, $echo = true, $target = false) {
	if($wrapper_classes) {
	...
	$output .= '<li class="breadcrumbs__item"><a href="'. home_url() .'" class="breadcrumbs__anchor">'. __('Home', 'algorigin-theme') .'</a></li>';

	$target_type = '';
	if($target) {
		$target_post = get_post($target);
		$target_type = $target_post->post_type;
	}
	
	if(is_page() || $target_type === 'page') {
	...
	} else if(is_singular('post') || $target_type === 'post') {
	...
	} else if(is_singular('product') || $target_type === 'product') {
		...
		$term_url = get_term_link($current_term, 'category');
		$term_title = get_the_archive_title($current_term);
		$output .= '<li class="breadcrumbs__item"><a href="'. $term_url .'" class="breadcrumbs__anchor">'. $term_title .'</a></li>';
...
function theme_language_switcher($wrapper_classes = false) {
	if(function_exists('icl_object_id')) {
        ...
        $global_languages = [ 'en', 'de', 'fr' ];
		if(!empty($languages)) {
			echo '<div class="'. $wrapper_classes .'language-switcher select">';
			echo '<select id="select-language" class="select__input js-nav-select">';
            $new_languages = [];
            foreach ($languages as $key => $l) {
                if ( USER_REGION != 'CH' && in_array( $key, $global_languages ) ){
                    $new_languages[] = $l;
                } else if ( USER_REGION == 'CH' && !in_array( $key, $global_languages )){
                    $new_languages[] = $l;
                }
            }
			foreach($new_languages as $l) {
				$selected = '';
				if($l['active'] === '1') {
					$selected = ' selected';
				}
				echo '<option value="'. $l['url'] .'"'. $selected .'>'. $l['translated_name'] .'</option>';
			}
			echo '</select>';
			echo '</div>';
		}
...
function theme_get_wp_image($attachment_id, $image_size = false, $classes = '', $alt_fallback = false, $alt_override = false, $data_attributes = [], $title_text = false) {
	...
	if(!empty($title_text)) {
		$title_attribute = ' title="'. $title_text .'"';
		$image = substr_replace($image, $title_attribute, -2, 0);
	}
...
function trim_trailing_zeroes($number) {
