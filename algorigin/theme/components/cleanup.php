...
function theme_cleanup_excerpt_more($more) {
	global $post;
	return '...  <a class="excerpt-read-more" href="'. get_permalink( $post->ID ) . '" title="'. __( 'Read ', 'algorigin-theme' ) . esc_attr( get_the_title( $post->ID ) ).'">'. __( 'Read more &raquo;', 'algorigin-theme' ) .'</a>';
}
...
if(!ENABLE_WP_RESPONSIVE_IMAGES) {
    // REMOVE STATIC WIDTH AND HEIGHT ATTRIBUTES FROM WP IMAGES
    function theme_remove_wp_attachment_image_size_attributes( $html ) {
        $html = preg_replace( '/(width|height)="\d*"\s/', "", $html );
        return $html;
    }
    add_filter( 'post_thumbnail_html', 'theme_remove_wp_attachment_image_size_attributes', 10 );
    add_filter( 'image_send_to_editor', 'theme_remove_wp_attachment_image_size_attributes', 10 );
    add_filter( 'image_send_to_editor', 'theme_remove_wp_attachment_image_size_attributes', 10 );

    // REMOVE SRCSET FROM WP IMAGES
    function theme_disable_image_srcset( $sources ) {
        return false;
    }
    add_filter( 'wp_calculate_image_srcset', 'theme_disable_image_srcset' );
}
